...
async def _api_events(request: web.Request) -> web.Response:
    """Return recent optimizer events — improvements + collapsed non-improvement counts + migrations."""
    node = request.app["_doin_node"]
    ...
    if olap_conn:
        try:
            ...
            # Collapse consecutive non-improvements into a single count entry
            i = 0
            while i < len(rows):
                row = rows[i]
                if row["is_improvement"]:
                    events.append({
                        "type": "improvement",
                    ...
                    # Count consecutive non-improvements
                    start = i
                    while i < len(rows) and not rows[i]["is_improvement"]:
                        i += 1
                    count = i - start
                    events.append({
                        "type": "no_improvement",
                        "rounds_skipped": count,
                        "from_round": rows[start]["round_number"],
                        "to_round": rows[i - 1]["round_number"],
                        "timestamp": rows[i - 1]["timestamp_utc"],
                        "domain_id": rows[start]["domain_id"],
                    })
        except Exception:
            pass

    # Add migration events from chain (OPTIMAE_ACCEPTED blocks from other peers)
    try:
        my_peer_id = node.identity.peer_id
        chain_db = getattr(node, "_chain_db", None) or getattr(node, "chaindb", None)
        if chain_db:
            height = chain_db.height
            for idx in range(max(0, height - 100), height):
                block = chain_db.get_block(idx)
                if block and getattr(block, "block_type", "") == "OPTIMAE_ACCEPTED":
                    submitter = getattr(block, "submitter", "")
                    if submitter and submitter != my_peer_id:
                        payload = getattr(block, "payload", {}) or {}
                        events.append({
                            "type": "migration",
                            "round": idx,
                            "from_peer": submitter[:12],
